...
trait DatatableAttribute {
    ...
    public function action_butttons($model) {
        ...
        if (auth()->user()->can($this->getRouteName().'_update',$model)) {
            $buttons['edit'] = '<a class="btn btn-primary btn-xs btn-edit" href="'.$this->getRouteName().'/'.$model->id.'/edit" data-id="'.$model->id.'" data-toggle="tooltip" title="'.trans('messages.edit').'" data-original-title="'.trans('messages.edit').'"><i class="fa fa-pencil"></i></a>';
        }
